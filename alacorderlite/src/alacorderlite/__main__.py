...
tqdm.pandas(ncols=80,bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} cases')
warnings.filterwarnings('ignore')
...
sg.theme("DarkBlack")
sg.set_options(font="Default 13")
...
archive_layout = [
    ...
    [sg.Text("""Case text archives require a fraction of the storage capacity and processing time used to process PDF directories. Before exporting your data to tables, create an archive with supported file extensions .pkl.xz, .json(.zip), and .csv(.zip). Once archived, use your case text archive as an input for multitable or single table export.""",size=(50,None), font='Default 14')],
...
append_layout = [
    ...
    [sg.Text("""Case text archives require a fraction of the storage capacity and processing time used to process PDF directories. Before exporting your data to tables, create an archive with supported file extensions .pkl.xz, .json(.zip), and .csv(.zip). Once archived, use your case text archive as an input for multitable or single table export.""",auto_size_text=True, font='Default 14', size=(50,None))],
...
window = sg.Window(title="ALACORDER (graphical beta)", layout=layout, finalize=True)
...
while awaiting_input:
    ...
    elif event == "TB-COMPLETE-":
        sg.popup_ok("Alacorder completed table export.")
    elif event == "MA-COMPLETE-":
        sg.popup_ok("Alacorder completed text archive.")
    elif event == "SQ-COMPLETE-":
        sg.popup_ok("Alacorder completed search query.")
    elif event == "MQ-COMPLETE-":
        sg.popup_ok("Alacorder marked collected cases in search query.")
    elif event == "AA-COMPLETE-":
        sg.popup_ok("Alacorder appended archives successfully.")
